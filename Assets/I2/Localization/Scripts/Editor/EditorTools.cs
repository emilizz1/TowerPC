...
{	
	public class GUITools
	{
		...
		public static float FloatAngle(Rect rect, float value, float snap, float min, float max)
		{
			...
			{
				if (UnityEngine.Event.current.type == UnityEngine.EventType.MouseDown && knobRect.Contains(UnityEngine.Event.current.mousePosition))
				{
					GUIUtility.hotControl = id;
					mAngle_lastMousePosition = UnityEngine.Event.current.mousePosition;
				}
				else if (UnityEngine.Event.current.type == UnityEngine.EventType.MouseUp && GUIUtility.hotControl == id)
					GUIUtility.hotControl = -1;
				else if (UnityEngine.Event.current.type == UnityEngine.EventType.MouseDrag && GUIUtility.hotControl == id)
				{
		...
		public static float AngleCircle(Rect rect, float angle, float snap, float min, float max, Texture background=null, Texture knobLine=null)
		{
			...
			if (UnityEngine.Event.current != null && GUIUtility.hotControl<=0 &&  (UnityEngine.Event.current.type== UnityEngine.EventType.MouseDown ||
				UnityEngine.Event.current.type== UnityEngine.EventType.MouseDrag) && knobRect.Contains(UnityEngine.Event.current.mousePosition))
			{
